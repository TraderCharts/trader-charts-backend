...
import { INTEGER, STRING, BOOLEAN } from '../../adapters/DAOSequelizeManager';
...
const Alert = () => {
  ...
  const syncFixtures = async () => {
    let alertCount = await _model.count();
  ...
  const getAlerts = async (res, { offset, limit } = {}, where = {}) => {
  ...
  const updateAlert = async (id, data) => {
    ...
    const alertsTargetNegotiableInstrumentsModel = new AlertsTargetNegotiableInstruments().model;
