...
export const syncFixtures = async () => {
  ...
  let alertCount = await collection.count();
  ...
  if (alertCount === 0) {
    ...
    alertsFixtures = alertsFixtures.map(async (elem) => {
      ...
      let ace1 = await aceCollection.findOne(aceQuery1);
      let ace2 = await aceCollection.findOne(aceQuery2);
      let aco = await acoCollection.findOne(acoQuery);
      let u = await uCollection.findOne(uQuery);
      let ni = await niCollection.findOne(niQuery);
      let newElem = {
        ...elem,
