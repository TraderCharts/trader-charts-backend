...
describe("Tutorial REST API", function () {
    before(function (done) {
        ...
        //const sequelize = new Sequelize('postgres://postgres:postgres@127.0.0.1/api');
        done();
    });
    describe("Create photo", function () {
        it("returns the created resource on success", function (done) {
            ...
            request(app)
                ...
                .attach("avatar", __dirname + "/abe.jpg")
                .expect(201)
        ...
        it("returns 400, with error message on bad request", function (done) {
            ...
            request(app)
                .post("/photo")
                .attach("avatar", __dirname + "/bigPhoto.jpg")
                .field("album_id", badPhotoResource.album_id)
                .expect(400)
                .end(function (err, res) {
                    if (err) {
                    ...
                    assert.equal(res.body.errors[0], "Too large file");
                    done();
                });
        });
    });
